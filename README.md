...
### Option 1: Standalone Application (No installation required)
For the easiest experience, simply download and run our compiled executable.

1. Download `SIS_v8_Dashboard.exe` from the Releases tab.
2. Double-click the file. No Python installation is needed.
...
```
*(Required packages: `PySide6`, `numpy`, `pandas`, `astropy`, `scipy`, `xgboost`, `matplotlib`)*
...
Our system does not use "Black Box" AI to guess physical laws. Instead, we use a hybrid approach: **AI for speed, Physics for accuracy.**
...
After the AI sorts the data, our deterministic physics engine (`NEW_solar_analyzer_SIS_v8_fixed.py`) extracts four critical parameters to predict solar flares:
...
To guarantee scientific accuracy, the system uses stochastic **Monte-Carlo simulations**. It artificially adds noise to the data and runs 100+ iterations to calculate exact error margins (Uncertainty Estimation).

### 4. Automated LLM Reporting
Finally, the system compiles all the raw numbers and uses an integrated **LLM Agent (Gemini 2.5 Flash)** to generate a readable, human-friendly scientific report for the operator.

---

## 📁 Repository Structure
* `sis_gui_app_v2.py` — The front-end desktop application. Built with PySide6, it features a mission-control dashboard, live console, and file repository viewer. Runs the heavy math on background threads so the UI never freezes.
* `NEW_solar_analyzer_SIS_v8_fixed.py` — The heavy backend. Contains the XGBoost router, physical formulas, Monte-Carlo simulations, and LLM report generator.
