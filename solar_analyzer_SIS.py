...
try:
    ...
    print(
        "[WARN] scikit-learn not available.  SolarIntelligenceSystem disabled — "
...
class SolarIntelligenceSystem:
    ...
    Responsibilities
    ----------------
    1. Extract a rich 12-dimensional feature vector from each Stokes V profile.
    ...
    ----------
    classifier_path : str
        Path to the persisted ``RandomForestClassifier``.
    regressor_path : str
        Path to the persisted ``RandomForestRegressor``.
    scaler_path : str
        Path to the persisted ``StandardScaler``.
    is_trained : bool
        ``True`` once models are loaded from disk or trained on-the-fly.
    """
    ...
    def __init__(
        ...
        if all(os.path.exists(p) for p in all_paths):
            ...
            print(
                "[SIS] No pre-trained models found.  "
    ...
    def _load_models(self) -> None:
        ...
        try:
            ...
            print(
                f"[SIS] Loaded pre-trained models from disk "
    ...
    def _save_models(self) -> None:
        ...
        try:
            ...
            print(
                f"[SIS] Saved models → {self.classifier_path}, "
    ...
    def _extract_features(
        ...
        7  ``symmetry_correlation`` Cross-correlation of left/right halves
                                    around centre.
        ...
        wavelengths : np.ndarray
            Calibrated wavelength axis (same length as ``stokes_v``).
        center_idx : int
        ...
        # Off-lobe noise estimate — exclude sigma-search window so signal lobes
        # do not bias the noise floor.
        ...
        if len(off_lobe) >= 5:
            noise_sigma = 1.4826 * float(median_abs_deviation(off_lobe))
        else:
            noise_sigma = 1.4826 * float(median_abs_deviation(stokes_v))
        noise_sigma = max(noise_sigma, 1e-12)

        # Peak structure on smoothed profile with noise-aware prominence.
        # Using ``prominence = max(4*sigma_noise, 0.20 * max_amp)`` prevents
        # noise ripples from being counted as sigma-component peaks.
        v_smoothed = savgol_filter(
            stokes_v, SMOOTHING_WINDOW_SIZE, SMOOTHING_POLY_ORDER
        )
        prominence_thresh = max(4.0 * noise_sigma, 0.20 * max_amp)
        peaks_pos, _ = find_peaks(v_smoothed, prominence=prominence_thresh)
        peaks_neg, _ = find_peaks(-v_smoothed, prominence=prominence_thresh)
        n_pos = len(peaks_pos)
        n_neg = len(peaks_neg)
        ...
        # Symmetry — anti-symmetry is expected for a Zeeman Stokes V profile.
        try:
        ...
        # SNR estimate using off-lobe noise, consistent with heuristic labeller
        features[8] = max_amp / noise_sigma

        features[9] = float(n_pos)
        features[10] = float(n_neg)

        # Asymmetry index
        v_max = float(np.max(stokes_v))
        v_min = float(np.min(stokes_v))
        denom = abs(v_max) + abs(v_min)
        features[11] = abs(v_max + v_min) / denom if denom > 0 else 0.0
    ...
    def _classify_heuristic_3class(
        self,
        stokes_v: np.ndarray,
        center_idx: int,
    ) -> int:
        ...
        --------------------------------
        1. **ANOMALY** — Physically distinguishable multi-lobe structure
           (>=2 peaks on the *same* polarity side, total >=3) at SNR > 5,
           **or** extreme single-polarity amplitude asymmetry at SNR > 6,
           **or** pathological skewness (``|skew| > 4``) at SNR > 5.
        2. **CLEAR** — Exactly 1 positive *and* 1 negative lobe on opposite
           sides of centre with SNR > 3.
        3. **NOISY** — Everything else (low SNR, undecidable structure,
           borderline cases).

        Threshold rationale
        -------------------
        All peak detection uses a Savitzky-Golay smoothed copy of the profile
        with a noise-aware prominence threshold::

            prom = max(4 * sigma_off_lobe, 0.20 * max_amp)

        where ``sigma_off_lobe`` is estimated exclusively from spectral pixels
        **outside** the ``+/-SEARCH_WINDOW_HALF_PIX`` sigma-component search
        window.  This prevents the following failure modes:

        * With the old threshold (``0.15 * max_amp``, raw profile), Gaussian
          noise at SNR ~5 produced 30-60 spurious peaks that trivially
          satisfied the ">=3 peaks" anomaly rule.
        * Raising the SNR floor for the multi-peak rule to 5.0 (from 3.0)
          ensures extra lobes are physically resolvable, not noise.
        * The skewness threshold is raised 2.5 -> 4.0 because genuine Zeeman
          profiles at lobe-amplitude asymmetry <= 30 % routinely show
          ``|skew|`` up to 2.0-2.5 from noise alone.
        * The asymmetry-index threshold is tightened 0.70 -> 0.80 with SNR
          raised 4.0 -> 6.0 to require confident detection before flagging.
        ...
        -------
        int
            ``CLASS_CLEAR`` (1), ``CLASS_NOISY`` (0), or ``CLASS_ANOMALY`` (2).
        """
        from scipy.stats import skew as scipy_skew

        max_amp = np.max(np.abs(stokes_v))

        # Off-lobe noise estimate — exclude centre +/- SEARCH_WINDOW_HALF_PIX
        lobe_lo = max(0, center_idx - SEARCH_WINDOW_HALF_PIX)
        lobe_hi = min(len(stokes_v), center_idx + SEARCH_WINDOW_HALF_PIX + 1)
        off_lobe = np.concatenate([stokes_v[:lobe_lo], stokes_v[lobe_hi:]])
        if len(off_lobe) >= 5:
        ...
        # Peak detection on smoothed profile with noise-aware prominence
        v_smoothed = savgol_filter(
            stokes_v, SMOOTHING_WINDOW_SIZE, SMOOTHING_POLY_ORDER
        )
        prominence = max(4.0 * noise_sigma, 0.20 * max_amp)
        ...
        # (a) Multi-lobe: requires >=2 peaks on the SAME polarity side so that
        #     a simple 1+1 antisymmetric Zeeman pair can never trigger this rule
        #     even if one lobe splits marginally near the noise floor.
        has_multi_peaks = (
            (n_pos + n_neg) >= 3
            and (n_pos >= 2 or n_neg >= 2)  # same-side duplication required
            and snr > 5.0  # raised from 3.0 -> 5.0
        )
        # (b) Single-polarity amplitude asymmetry
        v_max, v_min = float(np.max(stokes_v)), float(np.min(stokes_v))
        denom = abs(v_max) + abs(v_min)
        asym_idx = abs(v_max + v_min) / denom if denom > 0 else 0.0
        has_strong_asymmetry = (
            asym_idx > 0.80 and snr > 6.0  # tightened from 0.70/4.0 -> 0.80/6.0
        )
        # (c) Pathological skewness inconsistent with any Gaussian lobe model
        skewness = abs(float(scipy_skew(stokes_v)))
        has_extreme_skew = (
            skewness > 4.0 and snr > 5.0  # raised from 2.5/3.5 -> 4.0/5.0
        )
        ...
        if snr > 3.0 and n_pos >= 1 and n_neg >= 1:
            ...
            # sides of the spectral line centre (Zeeman anti-symmetric signature)
            opposite_sides = any(
    ...
    def train_on_the_fly(
        ...
        """
        Train the classifier and B_guess regressor on a sample of the current
        FITS cube using heuristic labelling and Sigma-V ground truth.

        Classifier training
        -------------------
        All three classes (Clear / Noisy / Anomaly) are labelled using
        :meth:`_classify_heuristic_3class`.

        Regressor training
        ------------------
        Only ``"Clear"``-class samples where Sigma-V returns a valid ``B_G``
        are used as regression training points (features -> ``B_G``), keeping
        ``B_guess`` physically meaningful.

        Parameters
        ----------
        data_cube : np.ndarray
            Shape ``(4, n_slits, n_lambda)`` — full Stokes data cube.
        wavelengths : np.ndarray
            Calibrated wavelength axis.
        center_idx : int
            Pixel index of the reference line centre.
        n_samples : int, optional
            Number of randomly selected slit positions to train on.
        """
        _, n_slits, _ = data_cube.shape
        sample_indices = np.linspace(
            0, n_slits - 1, min(n_samples, n_slits), dtype=int
        )
        ...
        print(
            f"[SIS] Training on {len(sample_indices)} samples — "
            "building classifier + B_guess regressor ..."
        )
        ...
        if len(X_reg) >= 5:
            ...
            print(
                "[SIS]   Warning: too few Clear samples for regressor.  "
                "B_guess will be 0.0."
            )
            self.regressor = None
        ...
        # Training summary
        n_total_train = len(y_cls)
        unique, counts = np.unique(y_cls, return_counts=True)
        class_dist = dict(zip(unique, counts))

        print("[SIS] Classifier training complete.  Class distribution:")
        for lbl, cnt in zip(unique, counts):
            pct = 100.0 * cnt / n_total_train
            print(
                f"       {CLASS_LABELS[lbl]:8s} ({lbl}): {cnt:4d} samples  "
                f"({pct:5.1f} %)"
            )

        # Warn about extreme imbalance — symptom of misconfigured thresholds
        for code, name in CLASS_LABELS.items():
            if class_dist.get(code, 0) == 0:
                print(
                    f"[SIS] WARNING: class '{name}' has ZERO training samples.  "
                    "Delete *.joblib files and check threshold logic if unexpected."
                )

        anomaly_pct = 100.0 * class_dist.get(CLASS_ANOMALY, 0) / n_total_train
        if anomaly_pct > 50.0:
            print(
                f"[SIS] WARNING: Anomaly class is {anomaly_pct:.0f} % of training "
                "data.  If your data is typical solar observations this likely "
                "indicates the anomaly-detection thresholds are still too loose.  "
                "Delete *.joblib files to force retraining after adjusting "
                "SEARCH_WINDOW_HALF_PIX or the prominence/SNR constants."
            )
    ...
    def predict(
        ...
        b_guess : float
            Rapid regressor B-field estimate in Gauss.  Returns ``0.0`` if
            the regressor is unavailable or the predicted class is ``"Noisy"``.
        """
        ...
        try:
            ...
            # B_guess is only meaningful for Clear predictions
            b_guess = 0.0
...
class AIScientist:
    """
    LLM-powered scientific interpretation of analysis results (optional).

    Generates a human-readable report covering magnetic field statistics,
    AI routing distribution, and anomaly findings.  Requires a running
    Ollama instance; gracefully no-ops if unavailable.
    ...
    available : bool
        ``False`` if the ``ollama`` package could not be imported.
    """

    def __init__(self, model_name: str = AI_OLLAMA_MODEL) -> None:
    ...
    def generate_report(
        ...
        fits_filename: str,
        output_path: str = AI_REPORT_FILENAME,
    ) -> bool:
        """
        Generate a scientific report from the full results DataFrame.

        Covers magnetic field statistics, anomaly analysis, and the SIS
        routing distribution.

        Parameters
        ----------
        df_results : pd.DataFrame
            Full per-slit results table produced by :func:`analyze_fits_file`.
        fits_filename : str
            Base name of the source FITS file (used in the report header).
        ...
        -------
        bool
            ``True`` on success, ``False`` if generation failed or was skipped.
        """
        if not self.available:
            return False
        if not self._ollama_running():
            print(
                f"[AI Scientist] Ollama not running.  "
        ...
        valid_mask = df_results["B_G"].notna()
        n_valid = int(valid_mask.sum())

        if n_valid > 0:
            mean_b = df_results.loc[valid_mask, "B_G"].mean()
            std_b = df_results.loc[valid_mask, "B_G"].std()
            min_b = df_results.loc[valid_mask, "B_G"].min()
            max_b = df_results.loc[valid_mask, "B_G"].max()
        else:
            mean_b = std_b = min_b = max_b = 0.0

        # AI routing distribution
        route_counts = (
            df_results["AI_Route"].value_counts()
        ...
        cls_counts = (
            df_results["AI_Signal_Class"].value_counts()
...
1. Assessment of the observed magnetic field strengths
   (active region, quiet Sun, sunspot?).
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You are an expert solar physicist specialising in "
...
def analyze_fits_file(
    ...
    # -------------------------------------------------------------------------
    # Train SIS if no cached models were found
    # -------------------------------------------------------------------------
    ...
    for s_idx in range(n_slits):
        ...
        if signal_class == "Clear":
            ...
            if b_wfa is not None:
                ...
                print(
                    f"[SIS] Slit {s_idx}: Noisy + WFA failed -> "
            ...
            # Lightweight diagnostic plot for each anomalous profile
            fig, axes = plt.subplots(2, 1, figsize=(8, 5), sharex=True)
            axes[0].plot(calibrated_wavelengths, stokes_i, color="steelblue")
            axes[0].set_ylabel("Stokes I")
            axes[0].grid(True)
            axes[1].plot(calibrated_wavelengths, stokes_v, color="crimson", lw=1.2)
        ...
        # --- Step 3: Monte Carlo error estimation (Clear & Noisy with B) -----
        if (
        ...
        if signal_class != "Anomaly" and (row["sigma_found"] or row["suspect"]):
            ...
            ax1.plot(calibrated_wavelengths, stokes_i, label="I", color="steelblue")
            ...
            ax2.plot(calibrated_wavelengths, stokes_v, label="V_raw", alpha=0.55)
            ax2.plot(
                calibrated_wavelengths,
                v_smooth_plot,
                label="V_smooth",
            ...
            if row["wa_A"] is not None:
                ax2.axvline(row["wa_A"], color="magenta", ls="--", label="sigma-")
            if row["wb_A"] is not None:
                ax2.axvline(row["wb_A"], color="cyan", ls="--", label="sigma+")
            ...
            x_str = (
                f"{row['x_arcsec']:.1f}\""
                if row["x_arcsec"] is not None
                else "N/A"
            )
            title = (
                f"Slit {s_idx}  X={x_str}  B={b_str}  method={row['used']}\n"
                f"SIS: {signal_class} ({ai_confidence:.1f}%)  "
                f"B_guess={b_guess:.0f} G  Route -> {ai_route}"
            )
    ...
    if ai_scientist.available:
        ...
        ai_scientist.generate_report(
            df_results, os.path.basename(fits_path), report_path
        )
