...
class SolarIntelligenceSystem:
    ...
    def _extract_features(self,
        ...
        # ── Noise from off-lobe pixels only (mirrors physics engine) ──────
        # Exclude the sigma-search window (±SEARCH_WINDOW_HALF_PIX around
        # center_idx) so the signal lobes do not bias the noise floor.
        _lo = max(0, center_idx - SEARCH_WINDOW_HALF_PIX)
        _hi = min(len(stokes_v), center_idx + SEARCH_WINDOW_HALF_PIX + 1)
        _off_lobe = np.concatenate([stokes_v[:_lo], stokes_v[_hi:]])
        if len(_off_lobe) >= 5:
            noise_sigma = 1.4826 * float(median_abs_deviation(_off_lobe))
        else:
            noise_sigma = 1.4826 * float(median_abs_deviation(stokes_v))
        noise_sigma = max(noise_sigma, 1e-12)

        # ── Peak structure (on smoothed profile, noise-aware prominence) ──
        # Using prominence = max(4σ_noise, 0.20 × max_amp) prevents noise
        # ripples (which reach only ~15 % of max_amp at SNR ≈ 5) from being
        # counted as sigma-component peaks.  Raw stokes_v would yield 30–60
        # spurious peaks per profile and make every slit look anomalous.
        v_sm = savgol_filter(stokes_v, SMOOTHING_WINDOW_SIZE, SMOOTHING_POLY_ORDER)
        prominence_thresh = max(4.0 * noise_sigma, 0.20 * max_amp)
        peaks_pos, _ = find_peaks( v_sm, prominence=prominence_thresh)
        peaks_neg, _ = find_peaks(-v_sm, prominence=prominence_thresh)
        n_pos = len(peaks_pos)
        n_neg = len(peaks_neg)
        ...
        # ── SNR estimate (off-lobe noise, consistent with heuristic) ────
    ...
    def _classify_heuristic_3class(self,
        ...
        ------------------------------------
        1. ANOMALY  — Physically distinguishable multi-lobe structure (≥2 peaks
                      on the SAME polarity side, total ≥3) at SNR > 5, OR
                      extreme single-polarity amplitude asymmetry at SNR > 6, OR
                      pathological skewness (|skew| > 4) at SNR > 5.
        2. CLEAR    — Exactly 1 positive AND 1 negative lobe on OPPOSITE sides
                      of center, SNR > 3.
        3. NOISY    — Everything else (low SNR, undecidable structure, borderline).

        Threshold rationale
        -------------------
        All peak detection is performed on a Savitzky-Golay smoothed copy of
        the profile and uses a noise-aware prominence threshold:

            prom = max(4 × σ_off_lobe,  0.20 × max_amp)

        where σ_off_lobe is estimated exclusively from spectral pixels OUTSIDE
        the ±SEARCH_WINDOW_HALF_PIX sigma-component search window.  This is
        mandatory because:
        • With the old threshold (0.15 × max_amp, raw profile), gaussian noise
          at SNR ≈ 5 produces 30–60 spurious peaks that trivially satisfy the
          "≥3 peaks" anomaly rule, classifying every normal Zeeman profile as
          Anomaly.
        • Raising the SNR floor for the multi-peak rule to 5.0 (from 3.0)
          ensures the extra lobes are physically resolvable, not noise.
        • The skewness threshold is raised from 2.5 → 4.0 because genuine
          Zeeman profiles at lobe-amplitude asymmetry ≤30 % routinely show
          |skew| up to 2.0–2.5 from noise.
        • The asymmetry-index threshold is tightened from 0.70 → 0.80 with SNR
          raised from 4.0 → 6.0 to require confident detection of a single-
          polarity profile before flagging it.
        ...
        # ── Off-lobe noise estimate (same strategy as physics engine) ─────
        # Exclude center ± SEARCH_WINDOW_HALF_PIX so sigma lobes do not
        # inflate the noise floor and cause SNR under-counting.
        _lo = max(0, center_idx - SEARCH_WINDOW_HALF_PIX)
        _hi = min(len(stokes_v), center_idx + SEARCH_WINDOW_HALF_PIX + 1)
        _off = np.concatenate([stokes_v[:_lo], stokes_v[_hi:]])
        if len(_off) >= 5:
            noise_sigma = 1.4826 * float(median_abs_deviation(_off))
        else:
            noise_sigma = 1.4826 * float(median_abs_deviation(stokes_v))
        noise_sigma = max(noise_sigma, 1e-12)
        snr = max_amp / noise_sigma

        # ── Peak detection: smoothed profile + noise-aware prominence ─────
        v_sm = savgol_filter(stokes_v, SMOOTHING_WINDOW_SIZE, SMOOTHING_POLY_ORDER)
        prom = max(4.0 * noise_sigma, 0.20 * max_amp)
        peaks_pos, _ = find_peaks( v_sm, prominence=prom)
        peaks_neg, _ = find_peaks(-v_sm, prominence=prom)
        n_pos = len(peaks_pos)
        n_neg = len(peaks_neg)
        ...
        #     even if one lobe splits marginally into two peaks near the noise floor.
        has_multi_peaks = (
            (n_pos + n_neg) >= 3
            and (n_pos >= 2 or n_neg >= 2)   # same-side duplication required
            and snr > 5.0                     # raised from 3.0 → 5.0
        )
        # (b) Single-polarity amplitude asymmetry: profile dominated by one lobe
        ...
        has_extreme_skew = (
            sk > 4.0 and snr > 5.0           # raised from 2.5/3.5 → 4.0/5.0
        )
    ...
    def train_on_the_fly(self,
        ...
        for code, name in CLASS_LABELS.items():
            if class_dist.get(code, 0) == 0:
                print(f"[SIS] WARNING: class '{name}' has ZERO training samples. "
                      f"Delete *.joblib files and check threshold logic if this "
                      f"is unexpected.")
        anomaly_pct = 100.0 * class_dist.get(CLASS_ANOMALY, 0) / n_total_train
        if anomaly_pct > 50.0:
            print(f"[SIS] WARNING: Anomaly class is {anomaly_pct:.0f} % of training "
                  f"data.  If your data is typical solar observations this likely "
                  f"indicates the anomaly-detection thresholds are still too loose. "
                  f"Delete *.joblib files to force retraining after adjusting "
                  f"SEARCH_WINDOW_HALF_PIX or the prominence/SNR constants.")
